...
/**
 * 主从 Reactor 服务器实现（性能最优）
 */
public class JQuickThreadedSelectorServer implements JQuickThriftServer {
    ...
    public JQuickThreadedSelectorServer(JQuickServerConfig config, JQuickProtocolFactory protocolFactory, JQuickTransportFactory transportFactory) {
        this.config = config;
        this.protocolFactory = protocolFactory;
        this.transportFactory = transportFactory;
        this.multiplexedProcessor = new TMultiplexedProcessor();
        this.services = new ConcurrentHashMap<>();
        console.debug("JQuickThreadedSelectorServer created on port " + config.getPort() + ", selector threads=" + config.getSelectorThreads());
    }
    ...
    public void start() throws Exception {
        ...
        TThreadedSelectorServer.Args args = new TThreadedSelectorServer.Args(transport)
                ...
                .workerThreads(config.getMaxWorkerThreads())
                .acceptQueueSizePerThread(config.getAcceptQueueSizePerThread())
                .stopTimeoutVal(config.getStopTimeoutMs())
                .stopTimeoutUnit(java.util.concurrent.TimeUnit.MILLISECONDS);

        this.server = new TThreadedSelectorServer(args);
        this.running = true;
        console.info("Starting ThreadedSelector Thrift server on port " + config.getPort() + " with " + services.size() + " service(s), selector threads=" + config.getSelectorThreads() + ", worker threads=" + config.getMaxWorkerThreads());
        new Thread(() -> {
            server.serve();
            running = false;
            console.info("ThreadedSelector Thrift server on port " + config.getPort() + " stopped");
        }, "jquick-selector-server-" + config.getPort()).start();
    }

    @Override
    public void stop() {
        if (!running || server == null) return;
        console.info("Stopping ThreadedSelector Thrift server on port " + config.getPort());
        if (server.isServing()) {
            server.stop();
        }
        running = false;
    ...
    private TProcessor createProcessorForService(Object service) {
        ...
        if (interfaces.length == 0) {
            throw new RuntimeException("Service class " + serviceClass.getName() + " does not implement any interface");
        }
    ...
    public String getServerType() {
        return JQuickServerTypeEnums.selector.getCode();
    }
