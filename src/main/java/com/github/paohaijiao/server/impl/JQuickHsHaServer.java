 ...
 * 半同步半异步服务器实现
 */
public class JQuickHsHaServer implements JQuickThriftServer {
    ...
    public JQuickHsHaServer(JQuickServerConfig config, JQuickProtocolFactory protocolFactory, JQuickTransportFactory transportFactory) {
        this.config = config;
        this.protocolFactory = protocolFactory;
        this.transportFactory = transportFactory;
        this.multiplexedProcessor = new TMultiplexedProcessor();
        this.services = new ConcurrentHashMap<>();
        console.debug("JQuickHsHaServer created on port " + config.getPort());
    }
    ...
    public void start() throws Exception {
        ...
        this.running = true;
        console.info("Starting HsHa Thrift server on port " + config.getPort() + " with " + services.size() + " service(s), worker threads=" + config.getMaxWorkerThreads());
        new Thread(() -> {
            server.serve();
            running = false;
            console.info("HsHa Thrift server on port " + config.getPort() + " stopped");
        }, "jquick-hsha-server-" + config.getPort()).start();
    }

    @Override
    public void stop() {
        if (!running || server == null) return;
        console.info("Stopping HsHa Thrift server on port " + config.getPort());
        if (server.isServing()) {
            server.stop();
        }
        running = false;
    ...
    public String getServerType() {
        return JQuickServerTypeEnums.hsha.getCode();
    }
